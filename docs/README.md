...
- [x] 1,000원으로 시작하여 날마다 할인 금액 100원씩 증가, 총주문 금액에서 할인
...
- [x] 평일(일요일~목요일)에는 `디저트 메뉴`를 메뉴 1개당 `2,023`원 할인
- [x] 주말(금요일, 토요일)에는 `메인 메뉴`를 메뉴 1개당 `2,023`원 할인
- [x] 이벤트 달력에 별이 있으면 총주문 금액에서 `1,000`원 할인 (`매주 일요일 + 25일`)
- [x] 할인 전 총주문 금액이 `12만 원` 이상일 때, 샴페인 1개 증정
- [x] 샴페인을 주문한 경우 총결제 금액에서 제외, 주문하지 않은 경우 총혜택 금액에서만 사용
...
- [x] 메뉴 형식이 예시와 다른 경우
  - `"[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."`
- [x] 총 주문 금액이 10,000원 이하인 경우
  - 정상 동작하나 이벤트 적용 X

## 목표 달성

- 올해 12월에 지난 5년 중 최고의 판매 금액을 달성하기 위해 업셀링 전략 유도
  - [] 일정 금액 이상 구매 시 증정 문구 추가
- 12월 이벤트 참여 고객의 5%가 내년 1월 새해 이벤트에 재참여하는 것
  - [] 배지에 따라 각각 다른 새해 선물을 증정한다는 문구 추가
